...
export default function(router: IRouter) {
  router.post(
    ...
    async (
      ...
      return await exportSavedSearch(
        context,
        response,
        dataReport.report_params
      );
    }
  );
}

async function exportSavedSearch(
  context: RequestHandlerContext,
  response: KibanaResponse,
  reportParams: any
) {
  await populateMetaData(context, reportParams);
  return await generateReport(context, response);
}
